...
export class AgentOfficeSqliteStorage extends AgentOfficeStorageBase {
    ...
    `);
        const row = stmt.get(name, coworkerType);
        return {
            ...row,
            created_at: new Date(row.created_at),
        };
    ...
    `);
        const row = stmt.get(newCode, name);
        return {
            ...row,
            created_at: new Date(row.created_at),
        };
    ...
    `);
        const row = stmt.get(from, to, body);
        return {
            ...row,
            created_at: new Date(row.created_at + 'Z'), // Treat SQLite datetime as UTC
        };
    ...
    async listCronJobsForSession(sessionName) {
      ...
      ORDER BY name
    `);
        const rows = stmt.all(sessionName);
        return rows.map(row => ({
            ...row,
            created_at: new Date(row.created_at),
            last_run: row.last_run ? new Date(row.last_run) : null,
    ...
    `);
        const row = stmt.get(name, sessionName);
        if (!row)
            return null;
        return {
            ...row,
...
export function createSqliteStorage(databasePath) {
